...
/**
   ...
   * @returns {Promise<Array<ActiveDealDbEntry>>}
   */
export async function fetchDealsWithNoPayloadCid (pgPool, fromBlockHeight, limit) {
  const query = `SELECT * FROM active_deals WHERE payload_cid IS NULL AND activated_at_epoch >= ${fromBlockHeight} ORDER BY activated_at_epoch ASC LIMIT ${limit}`
  const result = await loadDeals(pgPool, query)
